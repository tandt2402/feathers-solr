...
function responseFind (filters, query, paginate, res) {
  ...
  if (!_.has(paginate, 'max') && !_.has(paginate, 'default')) {
    response = responseGet(res, true, query) || [];
  } else {
    response = {
    ...
    ['facets', 'highlighting', 'spellcheck', 'moreLikeThis', 'querqy_decorations', 'querqy.infoLog', 'nextCursorMark'].forEach(d => {
 ...
 */
function responseGet (res, allDocs = false, query = false) {
